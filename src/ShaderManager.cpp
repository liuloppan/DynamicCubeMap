...
const std::string ShaderManager::ReadFromFile(std::string fileName) {
...
GLuint ShaderManager::AttachShader(GLuint shaderType, std::string source) {
...
void ShaderManager::ConfigureShaderManager() {
	...
	if (isLinked == GL_FALSE) {
		...
		std::cerr << "Failed to link shaderManager : " << std::endl
			<< log << std::endl;
		exit(EXIT_FAILURE);
	}
	else {
...
const std::string ShaderManager::getShaderType(GLuint type) {
	std::string name;
	switch (type) {
...
GLuint ShaderManager::compile(GLuint type, GLchar const *source) {
